...
const FileStatus = () => {
  ...
  const handleSearch = async () => {
    try {
     const res = await API.post("/api/file/search", {
      ...
      setResults(res.data);
      setSelectedIndex(null); // Reset selected row
    } catch (error) {
  ...
  const handleViewFile = () => {
    ...
    if (!row || !row.file_path?.[0]) {
      alert("Selected row has no file");
      return;
    }

   window.open(`/api/file/view/${row.id}/0`, "_blank");
  };
  ...
  const handleDownloadFile = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "file_status.xlsx");
    ...
    <div className="fsadmin-container">
      ...
      <div className="fsadmin-box">
        ...
        <div className="fsadmin-row">
          ...
          <input
            type="text"
            className="fsadmin-small-input"
            value={owFrom}
            onChange={(e) => setOwFrom(e.target.value)}
          />
          <label>To:</label>
          <input
            type="text"
            className="fsadmin-small-input"
            value={owTo}
            onChange={(e) => setOwTo(e.target.value)}
          />
        </div>
        ...
        <div className="fsadmin-row">
          ...
          <input
            type="text"
            className="fsadmin-large-input"
            value={subject}
            onChange={(e) => setSubject(e.target.value)}
          />

          {/* One-time buttons */}
          <button className="fsadmin-search-btn" onClick={handleSearch}>
            Search
          </button>
          <button className="fsadmin-view-btn" onClick={handleViewFile}>
            View
          </button>
          <button className="fsadmin-download-btn" onClick={handleDownloadFile}>
            Download
          </button>
        </div>
        ...
        {/* ---------------- Results textarea ---------------- */}
        <textarea
          className="fsadmin-textarea"
          readOnly
          value={formatTable(results)}
          onClick={(e) => {
            // Determine row index based on clicked line
            const lineHeight = 18; // approx line height in px
            const clickY = e.nativeEvent.offsetY;
            const rowIndex = Math.floor(clickY / lineHeight) - 3; // adjust header lines
            if (rowIndex >= 0 && rowIndex < results.length) setSelectedIndex(rowIndex);
          }}
        ></textarea>

        <button className="fsadmin-print-btn" onClick={handlePrint}>
          PRINT
        </button>
      </div>
    </div>
